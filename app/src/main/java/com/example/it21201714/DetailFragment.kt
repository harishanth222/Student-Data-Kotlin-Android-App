...
class DetailFragment : Fragment() {
    ...
    override fun onCreateView(
       ...
       val rootView = inflater.inflate(R.layout.fragment_detail, container, false)
        ...
        val tv:EditText = rootView.findViewById(R.id.edtTxt)
        val btn: Button = rootView.findViewById(R.id.button4)

        btn.setOnClickListener {
            val studentId = tv.text.toString()

            if (studentId.isNotEmpty()) {
                CoroutineScope(Dispatchers.IO).launch {
                    val student = withContext(Dispatchers.IO) {
                        repository.getOne(studentId)

                    }
                    if(student!=null){
                        withContext(Dispatchers.Main){
                            val edtName: EditText = rootView.findViewById(R.id.edtName)
                            edtName.setText(student.item2)


                            val edtAge: EditText = rootView.findViewById(R.id.edtAge)
                            edtAge.setText(student.item3)
                        }
                    }else{
                        withContext(Dispatchers.Main) {
